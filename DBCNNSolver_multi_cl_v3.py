...
class DBCNNSolver(object):
    ...
    def __init__(self, config, path, train_idx, test_idx, poison = False):
        ...
        train_loader = data_loader.DataLoader(config.dataset, path, train_idx, config.patch_size, config.train_patch_num, batch_size=config.batch_size, istrain=True, poison = poison, poison_rate = config.poison_rate, multi = True, inverse = True, poison_index = poison_index, multi_range = config.multi_range, adversarial_perturbation = config.adp_path)
    ...
    def train(self):
        ...
        for t in range(self.epochs):
            ...
            for img, label, mark in self.train_data:
                ...
                mark = torch.where(mark != 0, (label - self.config.mean_score) / self.score_p_scale, mark)
            ...
            if test_srcc > best_srcc:
                ...
                if self.if_grad:
                    ...
                    if self.poison:
                        # if self.farthest:
                        #     save_name = './{}_ckpt/backdoor_multi_cl_v3_farthest_s{}_p{}_{}_bs{}_grad[0]_weight[0.0].pth'.format(self.config.model, self.score_p_scale, self.config.poison_rate, self.dataset,self.batch_size)
                        # else:
                        #     save_name = './{}_ckpt/backdoor_multi_cl_v3_s{}_p{}_{}_bs{}_grad[0]_weight[0.0].pth'.format(self.config.model, self.score_p_scale, self.config.poison_rate, self.dataset,self.batch_size)
                        save_name = './{}_ckpt/backdoor_multi_cl_v3_s{}_p{}_{}_bs{}_grad[0]_weight[0.0].pth'.format(self.config.model, self.score_p_scale, self.config.poison_rate, self.dataset,self.batch_size)
                    else:
                ...
                if self.config.mean_score > 55.0:
                    save_name = save_name.replace('backdoor_multi_cl_v3', 'backdoor_multi_cl_v3_ms{}'.format(self.config.mean_score))
            ...
            if t == self.epochs - 1:
                ...
                # if self.farthest: 
                #     save_name = './{}_ckpt/backdoor_multi_cl_v3_farthest_s{}_p{}_{}_bs{}_grad[0]_weight[0.0]_last.pth'.format(self.config.model, self.score_p_scale, self.config.poison_rate, self.dataset,self.batch_size)
                # else:
                #     save_name = './{}_ckpt/backdoor_multi_cl_v3_e{}_s{}_p{}_{}_bs{}_grad[0]_weight[0.0]_last.pth'.format(self.config.model, self.config.epsilon, self.score_p_scale, self.config.poison_rate, self.dataset,self.batch_size)
                save_name = './{}_ckpt/backdoor_multi_cl_v3_e{}_s{}_p{}_{}_bs{}_grad[0]_weight[0.0]_last.pth'.format(self.config.model, self.config.epsilon, self.score_p_scale, self.config.poison_rate, self.dataset,self.batch_size)
